return {
  ...
  { -- Git diff viewer
    ...
    keys = {
      ...
      { '<leader>gf', '<cmd>DiffviewFileHistory %<CR>', desc = 'Open Git [F]ile History' },
      { '<leader>gh', '<cmd>DiffviewFileHistory<CR>', desc = 'Open Git [H]istory (Global)' },
  ...
  },
  {
    ...
    config = function()
      ...
      vim.list_extend(ensure_installed, {
        ...
        'prettier', -- Used to format HTML/CSS/JS
      })
  ...
  },
  {
    ...
    opts = {
      ...
      scope = {
        enabled = true,
        highlight = 'IblScope', -- Use the default highlight group
        show_start = true, -- Show the start of the scope
        show_end = true, -- Show the end of the scope
        -- injected_languages = true, -- This is true by default
        -- show_exact_scope = false, -- This is false by default
      },
      exclude = {
        filetypes = {
          'help',
    ...
    init = function()
      ...
      vim.cmd 'highlight IblScope guifg=#525252 guibg=NONE' -- Define IblScope highlight group
    end,
  },
  -- DAP
  {
    ...
    config = function()
      ...
      -- Set DAP log level to DEBUG for more verbose output
      ...
      vim.keymap.set('n', '<leader>ds', function()
        require('dap').repl.open { type = 'log' }
      end, { desc = 'DAP: Show Log' })

      -- Breakpoint Persistence
      local DAP_BREAKPOINTS_FILE = vim.fn.stdpath 'data' .. '/dap_breakpoints.json'
      ...
      local function save_breakpoints()
        local status, result = pcall(function()
          ...
          vim.notify('Breakpoints saved to: ' .. DAP_BREAKPOINTS_FILE, vim.log.levels.DEBUG)
        end)

        if not status then
          vim.notify('Error saving breakpoints: ' .. result, vim.log.levels.ERROR)
          -- Also write to a file for persistent logging
          local error_log_file = vim.fn.stdpath 'cache' .. '/dap_breakpoints_error.log'
          vim.fn.writefile({ 'Error saving breakpoints: ' .. result }, error_log_file, 'a')
        end
      end

      -- Autocommands to save and load breakpoints
      vim.api.nvim_create_autocmd('VimLeavePre', {
        group = vim.api.nvim_create_augroup('DapBreakpointsPersistence', { clear = true }),
        callback = function()
          vim.notify('VimLeavePre autocommand fired, attempting to save breakpoints...', vim.log.levels.DEBUG)
          save_breakpoints()
        end,
      })

      -- Temporarily disabled load autocommand for debugging save issue
      -- vim.api.nvim_create_autocmd('VimEnter', {
      --   group = 'DapBreakpointsPersistence',
      --   callback = load_breakpoints,
      -- })

      -- Keybinding to clear all breakpoints
      vim.keymap.set('n', '<leader>Bc', function()
        require('dap').clear_breakpoints()
      end, { desc = 'DAP: Clear All Breakpoints' })

      -- Python configuration
      -- Adapter for 'debugpy' type in launch.json (used by VS Code)
      -- This uses the same executable and arguments as the 'python' adapter
      dap.adapters.debugpy = {
        type = 'executable',
        command = '/home/mstouffer/repos/company-cli/.venv/bin/python',
        args = { '-m', 'debugpy.adapter' },
      }

      dap.adapters.python = {
        type = 'executable',
        -- Directly specify the python executable path
        command = '/home/mstouffer/repos/company-cli/.venv/bin/python',
        args = { '-m', 'debugpy.adapter' },
      }
      ...
      dap.configurations.python = {
        {
          ...
          pythonPath = function()
            ...
            -- Check for a local .venv directory (common with uv)
            local uv_venv_path = cwd .. '/.venv/bin/python'
            if vim.fn.filereadable(uv_venv_path) then
              resolved_path = uv_venv_path
            end

            -- If not found, check VIRTUAL_ENV (set by mise or other venv managers)
            if not resolved_path then
              local venv = os.getenv 'VIRTUAL_ENV'
              if venv then
                resolved_path = venv .. '/bin/python'
              end
            end

            -- Fallback to global python if no venv is active
            if not resolved_path then
              resolved_path = '/usr/bin/python' -- Or wherever your global python is
            end

            vim.notify('DAP Python Path: ' .. resolved_path, vim.log.levels.INFO, { title = 'DAP Debug' })
